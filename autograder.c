...
void print_status(
    int solution_count,
    int input_count, // print status to print solutions like expected.out
    int results[solution_count][input_count],
    char names[solution_count][MAX_LINE_LENGTH], char *inputs[input_count]) {

  FILE *output_file = fopen("autograder.out", "w");
  if (output_file == NULL) {
    perror("Couldn't open autograder.out!");
    return;
  }

  for (int i = 0; i < solution_count; i++) {
    printf("%s", names[i]);
    if (fprintf(output_file, "%s", names[i]) < 0) {
    ...
    for (int j = 0; j < input_count; j++) {
      if (results[i][j] == 1) {
        ...
        if (fprintf(output_file, " %s(correct)", inputs[j]) < 0) {
          perror("couldn't print to file!");
          return;
        }
      } else if (results[i][j] == 0) {
        printf(" %s(incorrect)", inputs[j]);
        if (fprintf(output_file, " %s(incorrect)", inputs[j]) < 0) {
          perror("couldn't print to file!");
          return;
        }
      } else if (results[i][j] == -1) {
        printf(" %s(crashed)", inputs[j]);
        if (fprintf(output_file, " %s(crashed)", inputs[j]) < 0) {
          perror("couldn't print to file!");
          return;
        }
      }
...
int main(int argc, char *argv[]) {
  ...
  submissions_file =
      fopen("submissions.txt", "r"); // open submissions.txt for reading
  if (!submissions_file) {
  ...
  while (fgets(line, sizeof(line),
               submissions_file)) { // loop through submissions to see how many
                                    // lines there are
    solution_count += 1;
  }
  ...
  while (fgets(line, sizeof(line),
               submissions_file)) { // add every line into an array
    line[strcspn(line, "\n")] = '\0';
  ...
  if (solution_count % batch_size !=
      0) { // changing the number of batches based on the input
    total_batches += 1;
  }
  ...
  for (int i = 0; i < solution_count; i++) {
    if (i != 0 &&
        i % batch_size ==
            0) { // looping through solution count and copying to batches
      temp_batch_count += 1;
    }
    strncpy(batches[temp_batch_count][i % batch_size], solution_names[i],
            MAX_LINE_LENGTH);
  }
  ...
  for (int i = 0; i < total_batches; i++) {
    ...
    if (i == total_batches - 1 && solution_count % batch_size != 0) {
      real_batch_size =
          solution_count %
          batch_size; // accounting for variable size depending on B arguement
    }
  ...
  for (int i = 0; i < solution_count; i++) {
    for (int j = 0; j < solution_count; j++) {
      solution_results[i][j] =
          0; // changing results based on amount of solutions
    }
  }
        ...
        for (int i = 0; i < real_batch_size; i++) {
          if (batch_pids[i] != -1) {
            if (WIFEXITED(batch_statuses[i])) {
              ...
              } else if (WEXITSTATUS(batch_statuses[i]) ==
                         1) { // check if incorrect
                solution_results[batch_size * batch_number + i][input_number] =
                    0;
              ...
              printf("%d exited with %d \n", batch_pids[i],
                     WEXITSTATUS(batch_statuses[i])); // print id
              finished_processes += 1;
              batch_pids[i] = -1;
            } else if (WIFSIGNALED(batch_statuses[i]) &&
                       WTERMSIG(batch_statuses[i]) == 11) {
              solution_results[batch_size * batch_number + i]
                              [input_number] = // seg fault
                  -1;
